...
export function useRealTimeDetection() {
   ...
   */
  const initialize = ({ 
    serverUrl = 'https://monitor-backend.jetcamstudio:5000', 
    autoConnect = true,
    preferences = {}
  } = {}) => {
    if (socket.value) {
  ...
  const shouldShowDetection = (detection) => {
    ...
    if ((eventType === 'stream_started' || eventType === 'stream_ended') && !showStreamUpdates.value) {
      return false;
    }
  ...
  const showDetection = (detection) => {
    ...
    if (eventType === 'stream_started' || eventType === 'stream_ended') {
      ...
      const message = eventType === 'stream_started' 
      ...
      if (event.key === 'notificationPreferences') {
        try {
          ...
          if (preferences.types) {
            showKeywordDetections.value = preferences.types.keyword_detected !== false;
            showObjectDetections.value = preferences.types.object_detected !== false;
            showStreamUpdates.value = preferences.types.stream_started !== false && 
