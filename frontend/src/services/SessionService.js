...
const API_BASE_URL = 'https://monitor-backend.jetcamstudio:5000/api';
...
class SessionService {
  ...
  async handleNetworkChange() {
    ...
    if (!this.offlineMode && this.reconnectAttempts > 0) {
      ...
      console.log('Network connection restored. Attempting to restore session...');
    ...
    if (this.offlineMode) {
      ...
      this.toast.warning('Network connection lost. Some features may be unavailable.');
    }
  }
  ...
  async checkSession() {
    ...
    try {
      ...
      if (!navigator.onLine || (error.message && error.message.includes('Network Error'))) {
      ...
      return {
        authenticated: false,
        error: 'Session check failed. ' + (error.response?.data?.message || error.message || 'Unknown error')
      };
    } finally {
      this.isCheckingSession = false;
    }
  ...
  startSessionRefresh() {
    ...
    console.log(`Starting session refresh interval (every ${SESSION_REFRESH_INTERVAL/1000} seconds)`);
  ...
  scheduleOfflineRetry() {
    ...
    console.log(`Scheduling offline retry attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts} in ${OFFLINE_RETRY_DELAY}ms`);
    
    setTimeout(async () => {
      // Check if we're back online
      if (navigator.onLine) {
        console.log('Network appears to be back online, attempting session check');
        await this.checkSession();
      } else {
