...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fetchAgents = async () => {
      ...
      errorMessage.value = ''
      try {
        console.debug('Fetching agents from     http://localhost:5000api/agents')
        const response = await axios.get('     http://localhost:5000api/agents', {
          headers: {
    ...
    const connectSSE = (jobId) => {
      ...
      const sseUrl = `     http://localhost:5000api/streams/interactive/sse?job_id=${jobId}`
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const response = await axios.post('     http://localhost:5000api/streams/interactive', form.value, {
          headers: {
            'Accept': 'application/json',
