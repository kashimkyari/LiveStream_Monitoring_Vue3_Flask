...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fetchAgents = async () => {
      ...
      errorMessage.value = ''
      try {
        console.debug('Fetching agents from http://54.86.99.85:5000/api/agents')
        const response = await axios.get('http://54.86.99.85:5000/api/agents', {
          headers: {
    ...
    const connectSSE = (jobId) => {
      ...
      const sseUrl = `http://54.86.99.85:5000/api/streams/interactive/sse?job_id=${jobId}`
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const response = await axios.post('http://54.86.99.85:5000/api/streams/interactive', form.value, {
          headers: {
            'Accept': 'application/json',
