...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fetchAgents = async () => {
      ...
      errorMessage.value = ''
      try {
        console.debug('Fetching agents from https://54.86.99.85:5000/api/agents')
        const response = await axios.get('https://54.86.99.85:5000/api/agents', {
          headers: {
    ...
    const connectSSE = (jobId) => {
      ...
      const sseUrl = `https://54.86.99.85:5000/api/streams/interactive/sse?job_id=${jobId}`
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const response = await axios.post('https://54.86.99.85:5000/api/streams/interactive', form.value, {
          headers: {
            'Accept': 'application/json',
